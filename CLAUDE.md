...
**Client** (`src/client`): vanilla TS SPA with a small hash router (`#/landing|slides|details|confirmed|farewell`). `main.ts` boots by fetching config, collecting device signals, calling `/api/identify`, and storing the outcome in `store.ts`. The journey is landing (identity + RSVP form, `rsvp-form.ts`) → confirmed (attending) or farewell (declined), which link to the slides and details. Screen order and gating live in `flow.ts` and the screens (every saved RSVP, first or edit, lands on `confirmed` or `farewell` per `destinationAfterRsvp`; only guests who have answered get the details link, and decliners only see the venue if `form.decline.showDetails` is set). `src/admin` is a separate bundle for the admin console.
